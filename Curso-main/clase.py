class Curso:
    ...
    def calcualarNotaMitad(self):
        datoMenoresMitad = 0
        datoMenoresMitad2 = 0
        for i in range(len(self.__notas)):
            menores = 0
            for i2 in range(len(self.__notas)):
                if self.__notas[i2] < self.__notas[i]:
                    menores += 1
            if menores == len(self.__notas)/2:
                datoMenoresMitad = self.__notas[i]
            elif menores == (len(self.__notas)/2)+0.5:
                datoMenoresMitad = self.__notas[i]
                datoMenoresMitad2 = self.__notas[i-1]
            elif menores == (len(self.__notas)/2)-0.5:
                datoMenoresMitad = self.__notas[i]
                datoMenoresMitad2 = self.__notas[i+1]
        if len(self.__notas)/2 == int:
            return datoMenoresMitad
        else:
            return f"{datoMenoresMitad} {datoMenoresMitad2}"